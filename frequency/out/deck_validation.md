...
- Findings: **118** (0 errors, 13 warnings)

## By category

| category | severity | count |
| --- | --- | --- |
| `ambiguous-sense` | warning | 107 |
| `duplicate-translation` | warning | 11 |

## `ambiguous-sense`

- **#  33** `պետք` → `needed, required / нужный, необходимый` — 2 POS senses; competing: [adj] needed, required / [noun] need, requirement
- **#  41** `մեկ` → `one / один` — 2 POS senses; competing: [num] one / [pron] someone, somebody
- **#  87** `հայ` → `Armenian / армянин, армянский` — 2 POS senses; competing: [adj] Armenian / [noun] Armenian
- **# 102** `առաջին [առաչին]` → `first, earliest / первый` — 2 POS senses; competing: [adj] first, earliest / [num] first
...
- **# 301** `ընթացքում` → `during` — 2 POS senses; competing: [adv] during / [noun] locative singular of ընթացք (əntʻacʻkʻ)
...
- **#  10** `նա (#10), ան (#193)` → `he, she / он, она` — 2 lemmas map to identical gloss
- **#  28** `հայերեն (#28), հայոց (#85)` → `armenian / армянский` — 2 lemmas map to identical gloss
- **# 394** `տեղի (#394), վայր (#937)` → `place` — 2 lemmas map to identical gloss
- **# 462** `հնչյուն (#462), ձայն (#504)` → `sound` — 2 lemmas map to identical gloss
- **# 490** `կրկին (#490), էլի (#828)` → `again` — 2 lemmas map to identical gloss
- **# 560** `ակ (#560), աղբյուր (#640)` → `spring, fountain, source of water` — 2 lemmas map to identical gloss
- **# 580** `ուժեղ (#580), ամուր (#1044)` → `strong` — 2 lemmas map to identical gloss
- **# 657** `բացել (#657), բանալ (#662)` → `to open` — 2 lemmas map to identical gloss
- **# 867** `հունարեն (#867), հունական (#1072)` → `greek` — 2 lemmas map to identical gloss
- **#1041** `միրգ [միրք] (#1041), պտուղ (#1054)` → `fruit` — 2 lemmas map to identical gloss
